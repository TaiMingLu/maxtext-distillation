...
- SFT models -> ACC evaluation (eval_sft_acc.sh)
...
def generate_run_all_yaml(tasks: list, output_dir: str) -> str:
    ...
    for task in tasks:
        ...
        if eval_type == "ppl":
            ...
            lines.append(f"    run: bash train/eval/eval_pretrain_ppl.sh {run_name} {checkpoint_step} {ckpt_type}")
        else:  # acc
            lines.append(f"    run: bash train/eval/eval_sft_acc.sh {run_name} {checkpoint_step}")
...
def main():
    ...
    parser.add_argument(
        ...
        default="all",
        help="Which eval types to generate: all, pretrain (PPL only), sft (ACC only)",
    )
    parser.add_argument(
        "--teacher-archs",
        nargs="+",
