...
def generate_script(
    ...
    # Script filename
    script_name = f"exp1_llama{student_arch}-{teacher_naming}-{alpha_str}-s{student_seed}.sh"

    # Run name and ID
    run_name = f"exp1_llama3.1-{student_arch}-{teacher_naming}-{alpha_str}-s{student_seed}"
    run_id = f"exp1_llama{student_arch}_finewebedu_distill_soft_{teacher_naming}_{alpha_str}_s{student_seed}"
...
def generate_run_all_yaml(script_infos: list, output_dir: str) -> str:
    ...
    for info in script_infos:
        ...
        lines.append(f"    run: bash train/exp1_expand/{info['script_name']}")
...
def main():
    ...
    for student_arch in args.student_archs:
        for teacher_arch in args.teacher_archs:
            for tokens in args.tokens:
                for alpha in args.alphas:
                    if args.dry_run:
                        teacher_naming = get_teacher_naming(teacher_arch, tokens, args.teacher_seed)
                        alpha_str = alpha_to_str(alpha)
                        script_name = f"exp1_llama{student_arch}-{teacher_naming}-{alpha_str}-s{args.student_seed}.sh"
