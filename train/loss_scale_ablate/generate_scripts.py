...
DEFAULT_TEACHER_ARCHS = ["1b"]
DEFAULT_TOKENS = ["50B"]
DEFAULT_TEACHER_SEED = 42
DEFAULT_ALPHAS = [0.2, 0.8, 1.0]
DEFAULT_STUDENT_SEED = 43
DEFAULT_LOSS_MODES = ["keep_full_ce", "keep_full_kd"]
...
def generate_script(
    ...
    Args:
        loss_mode: One of "keep_full_ce" or "keep_full_kd"
    """
    teacher_naming = get_teacher_naming(teacher_arch, tokens, teacher_seed)
    alpha_str = alpha_to_str(alpha)

    # Script filename
    script_name = f"{loss_mode}_llama1b-{teacher_naming}-{alpha_str}-s{student_seed}.sh"

    # Run name and ID
    run_name = f"{loss_mode}_llama3.1-1b-{teacher_naming}-{alpha_str}-s{student_seed}"
    run_id = f"{loss_mode}_llama1b_finewebedu_distill_soft_{teacher_naming}_{alpha_str}_s{student_seed}"
    ...
    # Generate the appropriate flag for the loss mode
    if loss_mode == "keep_full_ce":
        kd_loss_mode_flag = "kd_keep_full_ce=True"
    elif loss_mode == "keep_full_kd":
        kd_loss_mode_flag = "kd_keep_full_kd=True"
    else:
    ...
    content = SCRIPT_TEMPLATE.format(
        ...
        kd_loss_mode_flag=kd_loss_mode_flag,
    )
...
def generate_run_all_yaml(script_infos: list, output_dir: str) -> str:
    ...
    for info in script_infos:
        ...
        lines.append(f"    run: bash train/loss_scale_ablate/{info['script_name']}")
...
def main():
    ...
    parser.add_argument(
        "--loss-modes",
        nargs="+",
        default=DEFAULT_LOSS_MODES,
        choices=["keep_full_ce", "keep_full_kd"],
        help=f"Loss modes to generate (default: {DEFAULT_LOSS_MODES})",
    )
    parser.add_argument(
        "--output-dir",
        default=".",
    ...
    generated = []
    for teacher_arch, tokens, alpha, loss_mode in product(
        args.teacher_archs, args.tokens, args.alphas, args.loss_modes
    ):
        if args.dry_run:
            teacher_naming = get_teacher_naming(teacher_arch, tokens, args.teacher_seed)
            alpha_str = alpha_to_str(alpha)
            script_name = f"{loss_mode}_llama1b-{teacher_naming}-{alpha_str}-s{args.student_seed}.sh"
