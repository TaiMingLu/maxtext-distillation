...
def generate_script(
    ...
    # Script filename
    script_name = f"exp1_llama1b-{teacher_naming}-{alpha_str}-s{student_seed}.sh"

    # Run name and ID
    run_name = f"exp1_llama3.1-1b-{teacher_naming}-{alpha_str}-s{student_seed}"
    run_id = f"exp1_llama1b_finewebedu_distill_soft_{teacher_naming}_{alpha_str}_s{student_seed}"
...
def generate_run_all_yaml(script_infos: list, output_dir: str) -> str:
    ...
    for info in script_infos:
        ...
        lines.append(f"    run: bash train/exp1/{info['script_name']}")
