...
def generate_script(
    ...
    # Script filename
    script_name = f"exp2_llama1b-{teacher_naming}-{alpha_str}-s{student_seed}.sh"

    # Run name and ID
    run_name = f"exp2_llama3.1-1b-{teacher_naming}-{alpha_str}-s{student_seed}"
    run_id = f"exp2_llama1b_finewebedu_distill_soft_{teacher_naming}_{alpha_str}_s{student_seed}"
...
def generate_run_all_yaml(script_infos: list, output_dir: str) -> str:
    ...
    for info in script_infos:
        ...
        lines.append(f"    run: bash train/exp2_expand/{info['script_name']}")
...
def main():
    ...
    for teacher_arch, tokens, alpha in product(args.teacher_archs, args.tokens, args.alphas):
        if args.dry_run:
            ...
            alpha_str = alpha_to_str(alpha)
            script_name = f"exp2_llama1b-{teacher_naming}-{alpha_str}-s{args.student_seed}.sh"
