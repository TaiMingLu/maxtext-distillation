#!/bin/bash
#
# Accuracy evaluation for rebuttal checkpoints
# Same eval logic as train/eval/eval_sft_acc.sh, adapted for rebuttal paths
# NOTE: This is pretrain accuracy (no chat template), not SFT accuracy
#
# Usage:
#   ./eval_acc.sh <run_name> [checkpoint_step] [checkpoint_type]
#
# checkpoint_type:
...
#
# Examples:
#   ./eval_acc.sh exp3_baseline_s43 4999 rebuttal
#   ./eval_acc.sh gs://taiming_us_central1/rebuttal/baselines/llama3.1-1b-s43/24999/items 0 path
#
...
# Configuration — same as train/eval/eval_sft_acc.sh
HF_MODEL_PATH="/home/terry/gcs-bucket/rebuttal/hf_models/Llama-3.1-8B"
EVAL_RESULTS_DIR="/home/terry/gcs-bucket/rebuttal/eval_results/acc"
...
echo "========================"
echo "ACC Evaluation Configuration:"
...
echo "------------------------------------------------------------------"
echo "Evaluating ACC for ${RUN_NAME} at step ${CHECKPOINT_STEP}"
...
echo "Done evaluating ACC for ${RUN_NAME}"
