#!/bin/bash
# Exp 2: Pure KD with official Llama 3.2 1B teacher
# Student: 1.7B, 50B tokens, pure KD (alpha=1.0)
# NOTE: Teacher uses llama3.2-1b-official config (head_dim=64)

cd ~/maxtext

echo "========================"
echo "Exp 2: Official 1B teacher, pure KD"
...
export SEQ_LEN=8192
export BATCH_SIZE=2
export GRAD_ACCUM=2  # Teacher uses dot_product attention (head_dim=64), needs less memory per device
export LR=3.e-4
...
export TEACHER_MODEL_NAME='llama3.2-1b-official'
export KD_TEACHER_PARAMETERS_PATH="/home/terry/gcs-bucket/rebuttal/converted/llama3.2-1b-official/0/items"
