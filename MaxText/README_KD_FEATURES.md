...
MaxText’s training loop now ships with opt-in knowledge-distillation (KD) controls that let you (a) prune the teacher distribution with top-k or top-p (nucleus) filtering, (b) decide whether probability mass that is pruned should form an "OTHER" bucket, and (c) fall back to hard-label imitation when the teacher’s argmax is all you trust. All flags default to their legacy values, so pre-existing workloads behave exactly as before until you explicitly enable the new knobs.

## 2. Using KD Features

| Argument | Type / Default | High-Level Effect |
| --- | --- | --- |
...
## 3. Implementation Walkthrough (Line-by-Line)

### Config wiring — `MaxText/configs/base.yml`
...
### Runtime validation — `MaxText/pyconfig.py`
...
### KD utility helper — `MaxText/max_utils.py`
...
### Regression coverage — `MaxText/tests/max_utils_test.py`
